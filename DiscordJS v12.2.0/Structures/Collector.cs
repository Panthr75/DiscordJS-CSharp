...
namespace DiscordJS
{
    ...
    public abstract class Collector<T, O> where T : class, IHasID where O : CollectorOptions
    {
        ...
        public Collector(Client client, Func<T, Collection<Snowflake, T>, bool> filter, O options)
        {
            Client = client;
            Filter = filter;
            Options = options;
            Collected = new Collection<Snowflake, T>();
            Ended = false;
            _timeout = null;
            _idletimeout = null;
        ...
        public abstract Snowflake Collect(T arg1);
        ...
        {
            Snowflake collect = Collect(arg1);
            if (collect != null && (Filter == null || Filter(arg1, Collected)))
            {
                Collected.Set(collect, arg1);
    ...
    /// </summary>
    /// <typeparam name="T">The type this collector collects</typeparam>
    /// <typeparam name="P2">The second argument type emitted by the listener</typeparam>
    /// <typeparam name="O">The options for this collector</typeparam>
    public abstract class Collector<T, P2, O> where T : class, IHasID where O : CollectorOptions
    {
        /// <summary>
        /// The client that instantiated this Collector
        /// </summary>
        ...
        /// The filter applied to this collector
        /// </summary>
        public Func<T, P2, Collection<Snowflake, T>, bool> Filter { get; internal set; }
        ...
        /// </summary>
        /// <param name="arg1">The first arg the listener emits</param>
        /// <param name="arg2">The second arg the listener emits</param>
        public delegate void CollectEvent(T arg1, P2 arg2);
        ...
        /// <param name="arg1">The first arg the listener emits</param>
        /// <param name="arg2">The second arg the listener emits</param>
        public delegate void DisposeEvent(T arg1, P2 arg2);
        ...
        /// <param name="client">The instantiating Client</param>
        /// <param name="options">Options for this collector</param>
        public Collector(Client client, Func<T, P2, Collection<Snowflake, T>, bool> filter, O options)
        {
            Client = client;
            Filter = filter;
            Options = options;
        ...
        /// <param name="arg2">The second arg the listener emits</param>
        /// <returns>Data to insert into collection, if any</returns>
        public abstract Snowflake Collect(T arg1, P2 arg2);
        ...
        /// <param name="arg1">The first arg the listener emits</param>
        /// <returns>Key to remove from the collection, if any</returns>
        public abstract Snowflake Dispose(T arg1, P2 arg2);
        ...
        /// <param name="arg1">The first arg the listener emits</param>
        /// <param name="arg2">The second arg the listener emits</param>
        public void HandleCollect(T arg1, P2 arg2)
        {
            Snowflake collect = Collect(arg1, arg2);
            if (collect != null && (Filter == null || Filter(arg1, arg2, Collected)))
            {
                Collected.Set(collect, arg1);

                OnCollect?.Invoke(arg1, arg2);
                OnceCollect?.Invoke(arg1, arg2);
        ...
        /// <param name="arg1">The first arg the listener emits</param>
        /// <param name="arg2">The second arg the listener emits</param>
        public void HandleDispose(T arg1, P2 arg2)
        {
            if (!Options.Dispose) return;

            Snowflake dispose = Dispose(arg1, arg2);
            if (dispose == null || (Filter != null && !Filter(arg1, arg2, Collected)) || !Collected.Has(dispose)) return;
            Collected.Delete(dispose);

            OnDispose?.Invoke(arg1, arg2);
            OnceDispose?.Invoke(arg1, arg2);
            OnceDispose = null;
            CheckEnd();
        }
    ...
    public class CollectorCollectionException<T> : Exception where T : IHasID
    {
        ...
        /// The collection
        /// </summary>
        public Collection<Snowflake, T> Collection { get; }
